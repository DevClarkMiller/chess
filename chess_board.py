...
class Board:
    ...
    def in_check_after_move(self, move_og, move_dest):
        pass
    ...
    def in_check(self, color):
        king_coords = self.blackKingCoords if color == "b" else self.white_score

        for _, moves in self.possible_moves_dict.items(): # Iterate over each of the possible moves in the dict
            for move in moves:
                if move == king_coords:
                    return True
    ...
    def get_possible_moves(self):
        ...
        if self.active_player == "w":
            print(f"PLAYER IN CHECK: {king_in_check}")
        tiles = self.tiles
        moves = {}
            ...
            for col in range(0, TILES_IN_ROW):
                ...
                if piece and piece.color == self.active_player:
                    if not king_in_check:
                        ...
                        if piece.piece_c == "k":    # Only get moves of king if 
                            get_moves()
